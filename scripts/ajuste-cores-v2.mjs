...
// Move a quantidade exata de Aguardando Sublimacao para Sublimacao.
// Localiza o fim real da funcao pelo proximo marcador estavel, evitando
// deixar "};" residual dentro de App.jsx.
const inicioSub = source.indexOf('  const enviarParaSublimacao = (pedidoNum, produtoNome) => {');
const proximoMarcador = source.indexOf('  const moverParaAguardandoCostura =', inicioSub);
if (inicioSub !== -1 && proximoMarcador !== -1) {
...
`;
  source = source.slice(0, inicioSub) + novo + source.slice(proximoMarcador);
}

// Ao sair da Sublimacao, remove qualquer cor antiga. As cores sao definidas
// somente em Aguardando Costura.
...
console.log("NeoCooler: fluxo de quantidade entre sublimacao e costura protegido; cores definidas somente em Aguardando Costura.");
