...
aplicar("alocGrid", `alocGrid: { display: "grid", g
  };`, `alocGrid: { display: "grid", gridTemplateColumns: "1.2fr 0.8fr 1fr 0.8fr auto", gap: 8 },
};`);
aplicar("limparTudo", `setConfirmarLimpeza(false);const exportarXLSX = () => {`, `setConfirmarLimpeza(false);
  };

  const exportarXLSX = () => {`);

aplicar("estadoProdutoManual", `const [produto, setProduto] = useState(PRODUTOS[0]);
  const [qtd, setQtd] = useState(1);`, `const [produto, setProduto] = useState(PRODUTOS[0]);
  const [produtoManual, setProdutoManual] = useState("");
  const [passaPeloCorte, setPassaPeloCorte] = useState(true);
  const [qtd, setQtd] = useState(1);`);

aplicar("adicionarProdutoManual", `  const adicionarItem = () => {
...
aplicar("limparFormularioProdutoManual", `    if (dataEntregaForm) definirDataEntrega(pedido.trim(), dataEntregaForm);
    setQtd(1);`, `    if (dataEntregaForm) definirDataEntrega(pedido.trim(), dataEntregaForm);
    setQtd(1);
    setProdutoManual("");
    setProduto(PRODUTOS[0]);
    setPassaPeloCorte(true);`);

aplicar("campoProdutoManual", `            <Field label="Produto">
            ...
            <Field label="Qtd do pedido">`, `            <Field label="Produto">
              <select style={styles.input} value={produto} onChange={(e) => setProduto(e.target.value)}>
...
aplicar("tituloLancamento", `<h2 style={styles.formTitle}>Lançar item manualmente (entra no pré-corte)</h2>`, `<h2 style={styles.formTitle}>Lançar item manualmente</h2>`);
aplicar("botaoLancamento", `<button style={styles.addBtn} onClick={adicionarItem} disabled={!pedido.trim()}>Adicionar ao pré-corte</button>`, `<button style={styles.addBtn} onClick={adicionarItem} disabled={!pedido.trim() || (produto === "__MANUAL__" && !produtoManual.trim())}>{passaPeloCorte ? "Adicionar ao pré-corte" : "Adicionar sem passar pelo corte"}</button>`);
aplicar("avisoLancamento", `<p style={styles.aviso}>Assim que o Patrick cortar, marque a quantidade e o dia na aba Pré-Corte.</p>`, `<p style={styles.aviso}>Produtos novos podem ser escritos manualmente. Selecione se o item passa pelo corte; quando não passar, ele entra diretamente em Aguardando Sublimação.</p>`);

// O parser completo do PDF é instalado pelo patch-pdf-import.mjs.
aplicar("estadoImportacaoPdf", `  const [pdfGerando, setPdfGerando] = useState(false);`, `  const [pdfGerando, setPdfGerando] = useState(false);
  ...
  const confirmarImportacaoPdf = () => {
    const novos = pdfLinhas.filter((l) => l.incluir).map((l) => ({ id: uid(), pedido: String(l.pedido || "").trim(), produto: l.produto === "__MANUAL__" ? String(l.produtoManual || l.textoOriginal || "").trim() : l.produto, qtd: Math.max(1, Number(l.qtd) || 1), passaPeloCorte: l.passaPeloCorte !== false, etapa: l.passaPeloCorte !== false ? "pre_corte" : "aguardando_sublimacao", criadoEm: Date.now() })).filter((l) => l.pedido && l.produto);
    salvar([...itens, ...novos]);
    setMostrarImportacaoPdf(false);
    setPdfLinhas([]);
  };`);

aplicar("botaoImportarPdf", `<button style={styles.exportBtn} onClick={exportarXLSX} disabled={itens.length === 0}>⬇ Baixar planilha (.xlsx)</button>`, `<label style={{ ...styles.exportBtn, display: "flex", alignItems: "center", justifyContent: "center", cursor: importandoPdf ? "wait" : "pointer" }}><input type="file" accept="application/pdf,.pdf" style={{ display: "none" }} disabled={importandoPdf} onChange={(e) => { abrirImportacaoPdf(e.target.files?.[0]); e.target.value = ""; }} />{importandoPdf ? "Lendo PDF..." : "📥 Importar PDF do Trello"}</label><button style={styles.exportBtn} onClick={exportarXLSX} disabled={itens.length === 0}>⬇ Baixar planilha (.xlsx)</button>`);

aplicar("modalImportacaoPdf", `        {confirmarLimpeza && (`, `        {mostrarImportacaoPdf && (
