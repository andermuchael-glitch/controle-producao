...
if (!source.includes("PRODUTOS_SEM_CORTE_V16")) {
  ...
  if (!source.includes(anchor)) throw new Error("V16: marcador CORTADORES não encontrado.");
...
const newLoad = `          const migrados = carregados.map((i) => {\n            const nomeOriginal = String(i.produto || "").trim().toUpperCase();\n            const corrigirMousepad = String(i.pedido) === "11109" && nomeOriginal === "__MANUAL__" && Number(i.qtd) === 20;\n            const produtoCorrigido = corrigirMousepad ? "${PRODUTO}" : i.produto;\n            const semCorte = produtoSemCorteV16(produtoCorrigido);\n            const etapaOriginal = i.etapa || "costura";\n            return {\n              ...i,\n              produto: produtoCorrigido,\n              passaPeloCorte: semCorte ? false : (i.passaPeloCorte ?? true),\n              etapa: semCorte && etapaOriginal === "pre_corte" ? "aguardando_sublimacao" : etapaOriginal,\n              equipe: i.equipe || "Não decidido",\n              feito: i.feito ?? false,\n              conferido: i.conferido ?? false,\n            };\n          });\n          setItens(migrados);\n          if (JSON.stringify(migrados) !== JSON.stringify(carregados)) salvarValor(STORAGE_KEY, JSON.stringify(migrados)).catch(() => {});`;
...
if (source !== original) {
  ...
  log("V16 aplicado: MOUSEPAD ERGONÔMICO reconhecido, sem corte, e registros existentes corrigidos.");
} else {
