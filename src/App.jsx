...
export default function App() {
  ...
  const marcarCortado = (pedidoNum, produtoNome, restante) => {
    ...
    const etapaDestino = ["TOALHA PERSONALIZADO 70X40","TOALHA 80X30","TOALHA C/ CAPUZ G","TOALHA C/ CAPUZ M"].includes(String(produtoNome).trim().toUpperCase()) ? "aguardando_sublimacao" : "aguardando_sublimacao";
  ...
  const toggleFeito = (id) => {
    ...
    if (!alvo) return;
    if (!alvo.feito && ["TOALHA PERSONALIZADO 70X40","TOALHA 80X30","TOALHA C/ CAPUZ G","TOALHA C/ CAPUZ M"].includes(String(alvo.produto).trim().toUpperCase())) {
  ...
  const preCorteAgrupado = useMemo(() => {
    ...
    return Object.entries(grupos).map(([numero, its]) => {
      const linhas = its.map((it) => ({ produto: it.produto, total: it.qtd, cortado: 0, restante: it.qtd }));
      return { numero, linhas, totalGeral: linhas.reduce((s, l) => s + l.total, 0), cortadoGeral: 0, dataEntrega: getItensDataEntrega(numero) };
    }).filter((p) => p.numero.toLowerCase().includes(filtroPedido.toLowerCase()));
  }, [itens, pedidosMeta, filtroPedido]);
  const resumoProdutosCorte = useMemo(() => {
    const mapa = {};
    for (const p of preCorteAgrupado) for (const l of p.linhas) { if (!mapa[l.produto]) mapa[l.produto] = { numPedidos: 0, restante: 0 }; mapa[l.produto].numPedidos += 1; mapa[l.produto].restante += l.restante; }
    return Object.entries(mapa).map(([produto, v]) => ({ produto, ...v })).sort((a, b) => b.restante - a.restante);
  }, [preCorteAgrupado]);
  ...
  const aguardandoSublimacaoAgrupado = useMemo(() => {
    const grupos = {};
    for (const it of itens) if (it.etapa === "aguardando_sublimacao") { if (!grupos[it.pedido]) grupos[it.pedido] = {}; grupos[it.pedido][it.produto] = (grupos[it.pedido][it.produto] || 0) + it.qtd; }
    return Object.entries(grupos).map(([numero, produtos]) => ({
      numero,
      linhas: Object.entries(produtos).map(([prod, total]) => ({ produto: prod, total, restante: total, alocado: 0 })),
      totalGeral: Object.values(produtos).reduce((s, q) => s + q, 0),
      alocadoGeral: 0,
      dataEntrega: getItensDataEntrega(numero),
    })).filter((p) => p.numero.toLowerCase().includes(filtroPedido.toLowerCase()));
  }, [itens, pedidosMeta, filtroPedido]);
  const sublimacaoFiltrada = useMemo(() => {
    return Object.entries(itens.filter((i) => i.etapa === "sublimacao").reduce((acc, it) => { const d = it.dataSublimacao || "sem-data"; (acc[d] ||= []).push(it); return acc; }, {})).sort((a, b) => b[0].localeCompare(a[0])).map(([dia, its]) => [dia, its.filter((i) => filtroSublimador === "Todos" || i.sublimador === filtroSublimador).filter((i) => String(i.pedido).toLowerCase().includes(filtroPedido.toLowerCase()))]).filter(([, its]) => its.length);
  }, [itens, filtroSublimador, filtroPedido]);
  const agruparPorPedido = (etapa, campo) => {
    const grupos = {};
    for (const it of itens) if (it.etapa === etapa) (grupos[it.pedido] ||= []).push(it);
