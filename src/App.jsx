...
export default function App() {
  ...
  const salvarCorAguardandoCostura = async (id) => {
    ...
    try { await salvar(itens.map((i) => i.id === id ? { ...i, cor } : i)); setCosturaForm((x) => ({ ...x, [id]: { ...f, cor } })); setErro(""); }
    catch (e) { console.error(e); setErro("Não foi possível salvar a cor. Verifique a conexão com o Firebase."); }
  };
  const enviarParcialParaCostura = async (id) => {
    ...
    const restante = total - qtdEnviar, nova = [];
    for (const i of itens) { if (i.id !== id) nova.push(i); else { if (restante > 0) nova.push({ ...i, qtd: restante }); nova.push({ ...i, id: uid(), qtd: qtdEnviar, cor, equipe: f.equipe || EQUIPES[0], etapa: "costura", feito: false, conferido: false, criadoEm: Date.now() }); } }
    await salvar(nova); setCosturaForm((x) => { const y = { ...x }; delete y[id]; return y; }); setErro("");
  };
